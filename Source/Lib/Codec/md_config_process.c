...
static void svt_av1_qm_init(PictureParentControlSet* pcs) {
    ...
    if (pcs->frm_hdr.quantization_params.using_qmatrix) {
        ...
        case TUNE_VQ:
            ...
            pcs->frm_hdr.quantization_params.qm[PLANE_Y] = psy_get_qmlevel(
                base_qindex, min_qmlevel, max_qmlevel);
            pcs->frm_hdr.quantization_params.qm[PLANE_U] = psy_get_qmlevel
            (base_qindex + pcs->frm_hdr.quantization_params.delta_q_ac[PLANE_U],
                min_chroma_qmlevel,
                max_chroma_qmlevel);
            pcs->frm_hdr.quantization_params.qm[PLANE_V] = psy_get_qmlevel(
                base_qindex + pcs->frm_hdr.quantization_params.delta_q_ac[PLANE_V],
                min_chroma_qmlevel,
                max_chroma_qmlevel);
                break;
