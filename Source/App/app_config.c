...
#define BUILD_TYPE_STRING "debug"
    ...
    printf("SVT-AV1-Skibidi %s (" BUILD_TYPE_STRING ")\n", svt_av1_get_version());
#if defined(_WIN64) || defined(_MSC_VER) || defined(_WIN32)
    printf("Skibidi Release: %s\n", svt_hdr_get_version());
#else
    if (strcmp(svt_hdr_get_version(), "N/A")) {
        if (color) {
            printf("Skibidi Release: \x1b[32m%s\x1b[0m\n", svt_hdr_get_version());
        } else {
            printf("Skibidi Release: %s\n", svt_hdr_get_version());
        }
    } else {
        if (color) {
            printf("Skibidi Release: \x1b[38;5;248m%s\x1b[0m\n", svt_hdr_get_version());
        } else {
            printf("Skibidi Release: %s\n", svt_hdr_get_version());
        }
    }
